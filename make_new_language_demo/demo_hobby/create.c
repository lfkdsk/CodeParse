...
/*
   hbb_create_if_statement 创建if声明
   Expression *condition  if 条件
   Block *then_block if后的代码段
   ElseIf *elseif_list if－else列表
   Block *else_block else块
 */
...
/**
 * 前驱else-if list创建后面的else-if
 * @param  list 前驱列表
 * @param  add  添加的else-if
 * @return      else-if
 */
...
ElseIf *hbb_create_elseif(Expression *exp, Block *block){
        ...
        elseif = hbb_malloc(sizeof(ElseIf));
        elseif->condition = exp;
        elseif->block = block;
        elseif->next =  NULL;
...
/**
 * 创建语句块
 * @param  statement_list 语句列表
 * @return                语句块
 */
Block *hbb_create_block(StatementList *statement_list){
        Block *block;

        block = hbb_malloc(sizeof(Block));
...
Statement *hbb_create_return_statement(Expression *exp){
        ...
        st = alloc_statement(RETURN_STATEMNNT);
        st->u.expression_s = exp;
...
Statement *hbb_create_continue_statement(void){
        return alloc_statement(CONTINUE_EXPRESSION);
}
